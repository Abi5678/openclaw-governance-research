...
*Note: SARC [1] defines enforcement sites (pre-action, action-time, post-action, escalation) which constitute a form of ordering. However, SARC does not specify cross-module execution order or conflict arbitration among heterogeneous governance modules (e.g., how to resolve conflicts between authorization and budget constraints). OpenClaw-Govern extends SARC's ordering concept to multi-module composition with deterministic arbitration.*
