...
**AARM** (Autonomous Action Runtime Management) specifies runtime interception for AI-driven actions, including contextual evaluation, allow/deny/modify/defer/step-up decisions, and tamper-evident receipts for forensic reconstruction [14]. That makes AARM the closest precedent for the interception and receipt layer in a governed agent system. OpenClaw-Govern uses the same runtime-security intuition but asks a different question: when SARC-style constraints, authorization propagation, semantic guardrails, and async controls all fire on the same action, how do we order and reconcile them deterministically? In the delegated-remediation-conflict case, the step-up signal and the serialization signal must both remain visible in the trace even though the composed verdict is ESCALATE; the benchmark frames this as ROMA-style delegation plus AARM-style interception plus async serialization/conflict arbitration. Our contribution is therefore the composition layer above interception, not interception alone.

Prompt-injection and trace-analysis work provide an additional precedent for provenance-preserving governance: if a malicious request is laundered through a summary or adapter, the runtime trace must retain enough lineage to recover the original intent. AgentArmor-style trace reconstruction, AuthGraph-style provenance/authorization separation, VIGIL-style verify-before-commit, and trace-based assurance frameworks support that requirement, and the new `provenance_retention` metric operationalizes it in the evaluation. They motivate the evidence model rather than replacing OpenClaw-Govern's novelty, which remains the ordered composition and deterministic arbitration of heterogeneous modules.
